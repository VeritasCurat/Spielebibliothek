...
public class SchachKI2 {
	...
	private Integer[] minimax(int tiefe, String farbe, Integer[] zug, Schachbrett sb) {
		...
		if(tiefe == 0) {
			...
			if(invFarbe(farbe).equals("weiß")) a[4] =  -(kopie.figurenpunkte_weiß() - kopie.figurenpunkte_schwarz());
			else if(invFarbe(farbe).equals("schwarz")) a[4] = kopie.figurenpunkte_schwarz() - kopie.figurenpunkte_weiß(); 
			return a;
		}
		...
		for(int i=0; i<max_zuege.size(); i++) {
				...
				if(!kopie.bewegungPrüfen(max_zuege.get(i)[0],max_zuege.get(i)[1],max_zuege.get(i)[2],max_zuege.get(i)[3],farbe))continue;
